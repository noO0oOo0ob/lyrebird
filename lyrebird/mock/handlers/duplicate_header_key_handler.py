...
def duplicate_set_origin_header(origin_header, flow_header):
    if not flow_header:
        return

    raw_header = flow2origin(flow_header)
    header_dict = dict()
...
@numba.jit(nopython=False, forceobj=True)
def duplicate_origin2flow(raw_header):
    if not raw_header:
        return
...
trans_key_handlers = {
    'set-cookie': {
        'encode': _to_json_string,
        'decode': _to_json_object
    }
}
