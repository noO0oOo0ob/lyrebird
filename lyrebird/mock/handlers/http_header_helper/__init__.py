...
from ..duplicate_header_key_handler import *
...
def headers_origin2flow(origin_obj, output=None):
    _origin_headers = origin_obj.headers
    if not _origin_headers:
        return

    _headers = duplicate_origin2flow(_origin_headers)
...
@numba.jit(nopython=False, forceobj=True)
def headers_flow2origin(flow_obj, output=None, chain=None):
